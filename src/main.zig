...
pub fn run(allocator: std.mem.Allocator) !void {
    while (true) {
        ...
        if (app_state) |as| {
            switch (as) {
                .open => {
                    const selected_index = try app.open_selector_ui(allocator, projects, "action: OPEN");
                ...
                .delete => {
                    const selected_index = try app.open_selector_ui(allocator, projects, "action: DELETE");
                ...
                .add => {
                    ...
                    app.open_input(allocator, &name, "name: ", false) catch {
                        return;
                    };

                    var path: std.ArrayList(u8) = .empty;
                    defer path.deinit(allocator);
                    app.open_input(allocator, &path, "path: ", true) catch {
                        return;
                    };
                    const new_project: types.Project = .{ .name = name.items, .path = path.items };
                    try config.add_project(allocator, projects, new_project, constants.config_file_path);
                },
                .edit => {
                    const selected_index = try app.open_selector_ui(allocator, projects, "action: EDIT");
                    if (selected_index) |index| {
                        var name: std.ArrayList(u8) = .empty;
                        defer name.deinit(allocator);
